...
<body class="bg-gray-100">
  ...
  <div class="flex min-h-screen">
    ...
    <aside class="w-56 bg-green-200 p-6 flex flex-col justify-between shadow-md">
      <div>
        ...
        <nav class="flex flex-col gap-3 text-gray-800 text-sm">
          ...
          <a href="#" class="py-2 px-3 rounded-lg hover:bg-green-300 hover:translate-x-1 transition-all duration-200"><b>Jadwal Jaga</b></a>
    ...
    <main class="flex-1 p-8">
      <div class="mb-5"><a href="form/form_tambah_jadwalJaga.php" class="inline-block text-black hover:bg-green-500 border border-green-500 px-3 py-1 rounded-lg transition"><button>Tambah</button></a></div>
        ...
        <table class="w-full text-sm text-left border-t border-gray-200">
          <thead>
            <tr class="bg-sky-100 text-gray-700 uppercase text-xs tracking-wide">
              <th class="px-6 py-3">ID Jadwal Jaga</th>
            ...
            <?php
            include "koneksi.php";
            $query = "select j.ID_Jadwal_Jaga, j.shift, p.Nama_Perawat, k.No_Kamar from jadwal_jaga as j 
            left join perawat as p on j.ID_Perawat = p.ID_Perawat
            left join rawat_inap as r on j.ID_Rawat_Inap = r.ID_Rawat_Inap
            left join kamar as k on r.ID_Kamar = k.ID_Kamar
            order by j.ID_Jadwal_Jaga
            ";
            ...
            foreach($data as $item){
                ...
                <tbody class='divide-y divide-gray-100'>
                    ...
                    <td class='italic px-6 py-3'>".$item['ID_Jadwal_Jaga']."</td>
                    <td class='italic px-6 py-3'>".$item['shift']."</td>
                    <td class='italic px-6 py-3'>".$item['Nama_Perawat']."</td>
                    <td class='italic px-6 py-3'>".$item['No_Kamar']."</td>
                    <td class='italic px-6 py-3'>
                      <div class='grid grid-cols-2 gap-3'>
                        <a href='action/action_hapus.php?hapus=jadwalJaga&id=".$item['ID_Jadwal_Jaga']."' class='inline-block text-red-600 hover:text-white hover:bg-red-600 border border-red-600 px-3 py-1 rounded-lg transition'>Hapus</a>

                        <a href='edit/form_edit_jadwalJaga.php?id=".$item['ID_Jadwal_Jaga']."' class='inline-block text-green-600 hover:text-white hover:bg-green-600 border border-green-600 px-3 py-1 rounded-lg transition'>Edit</a>
                      </div>
                    </td>
                    </tr>
                </tbody>
